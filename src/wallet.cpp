...
// bdtcoin: total coins staked (non-spendable until maturity)
...
bool CWallet::CreateCoinStake(const CKeyStore& keystore, unsigned int nBits, int64_t nSearchInterval, int64_t nFees, CTransaction& txNew, CKey& key)
{
    ...
    if (txNew.vout.size() == 3)
    {
        ...
        txNew.vout[2].nValue = nCredit - txNew.vout[1].nValue;// wallet to wallet transaction
    }
    else
    {
...
// bdtcoin: check 'spent' consistency between wallet and txindex
// bdtcoin: fix wallet spent state according to txindex
...
// bdtcoin: disable transaction (only for coinstake)
