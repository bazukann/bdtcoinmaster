 ...
 * not found. */
static strmap_t *country_ibdtlus1_by_lc_code = NULL;
...
geoip_get_country(const char *country)
{
  ...
  ibdtlus1_ = strmap_get_lc(country_ibdtlus1_by_lc_code, country);
  if (!ibdtlus1_)
    return -1;

  idx = ((uintptr_t)ibdtlus1_)-1;
                ...
                const char *country)
{
  ...
  if (!ibdtlus1_) {
    ...
    strmap_set_lc(country_ibdtlus1_by_lc_code, country, (void*)(idx+1));
  } else {
    idx = ((uintptr_t)ibdtlus1_)-1;
  }
  {
...
init_geoip_countries(void)
{
  ...
  country_ibdtlus1_by_lc_code = strmap_new();
  strmap_set_lc(country_ibdtlus1_by_lc_code, "??", (void*)(1));
}
...
clear_geoip_db(void)
{
  ...
  strmap_free(country_ibdtlus1_by_lc_code, NULL);
  if (geoip_ipv4_entries) {
  ...
  country_ibdtlus1_by_lc_code = NULL;
