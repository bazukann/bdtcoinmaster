...
unsigned int nStakeMaxAge = 7 * 24 * 60 * 60; // 100 hours
...
const string strMessageMagic = "BDTCoin Signed Message:\n";
...
void SyncWithWallets(const CTransaction& tx, const CBlock* pblock, bool fUpdate, bool fConnect)
{
    if (!fConnect)
    {
        // bdtcoin: wallets need to refund inputs when disconnecting coinstake
        if (tx.IsCoinStake())
        {
            BOOST_FOREACH(CWallet* pwallet, setpwalletRegistered)
                if (pwallet->IsFromMe(tx))
                        ...
                        bool* pfMissingInputs)
{
    ...
    // bdtcoin: coinstake is also only valid in a block, not as a loose transaction
    if (tx.IsCoinStake())
...
int64_t GetProofOfStakeReward(int64_t nCoinAge, int64_t nFees)
{
    ...
    if(nBestHeight < 262800)
        nRewardCoinYear = 1.2 * nRewardCoinYear; //12% reward for 262800 blocks 1st  year
    else if(nBestHeight < 1051200)
        nRewardCoinYear = 1 * nRewardCoinYear; // 10% reward for 3153600 blocks  2 to 4 year
    else if(nBestHeight < 1839600)
        nRewardCoinYear = 0.8 * nRewardCoinYear; // 8% reward for 4730400 blocks 5 to 7 Year
    else
        nRewardCoinYear = 0.6 * nRewardCoinYear; // for rest 6%
...
// bdtcoin: find last block index up to pindex
    ...
    const CBlockIndex* pindexBlock, bool fBlock, bool fMiner)
{
    ...
    if (!IsCoinBase())
    {
        ...
        for (unsigned int i = 0; i < vin.size(); i++)
        {
            ...
            // bdtcoin: check transaction timestamp
...
bool CBlock::DisconnectBlock(CTxDB& txdb, CBlockIndex* pindex)
{
    ...
    // bdtcoin: clean up wallet after disconnecting coinstake
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    if (IsProofOfStake())
    {
        ...
        // bdtcoin: coin stake tx earns reward instead of paying fee
        uint64_t nCoinAge;
    ...
    // bdtcoin: track money supply and mint amount info
...
// bdtcoin: total coin age spent in transaction, in the unit of coin-days.
...
// bdtcoin: total coin age spent in block, in the unit of coin-days.
...
bool CBlock::AddToBlockIndex(unsigned int nFile, unsigned int nBlockPos, const uint256& hashProofOfStake)
{
    ...
    // bdtcoin: compute chain trust score
    pindexNew->nChainTrust = (pindexNew->pprev ? pindexNew->pprev->nChainTrust : 0) + pindexNew->GetBlockTrust();

    // bdtcoin: compute stake entropy bit for stake modifier
    if (!pindexNew->SetStakeEntropyBit(GetStakeEntropyBit()))
        return error("AddToBlockIndex() : SetStakeEntropyBit() failed");

    // bdtcoin: record proof-of-stake hash value
    pindexNew->hashProofOfStake = hashProofOfStake;

    // bdtcoin: compute stake modifier
...
bool CBlock::AcceptBlock()
{
    ...
    // bdtcoin: check pending sync-checkpoint
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
    ...
    // bdtcoin: ask for pending sync-checkpoint if any
    ...
    if (!mapBlockIndex.count(pblock->hashPrevBlock))
    {
        ...
        if (pfrom)
        {
            ...
            // bdtcoin: getblocks may not obtain the ancestor block rejected
    ...
    // bdtcoin: if responsible for sync-checkpoint send it
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "BDTCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "BDT Era starts from 17 August 2018";
        CTransaction txNew;
        txNew.nTime = 1543692000;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        //// debug print
        assert(block.hashMerkleRoot == uint256("91e6b89b98159ff9f72f9c04265d8d0d3baae124e8ef396a23a63de203303a74"));
        block.print();
        ...
        // bdtcoin: initialize synchronized checkpoint
...
unsigned char pchMessageStart[4] = { 0x23, 0xa3, 0xe7, 0xfd };
        ...
        BOOST_FOREACH(const CInv& inv, vInv)
        {
            ...
            if (inv.type == MSG_BLOCK)
            {
                ...
                if (mi != mapBlockIndex.end())
                {
                    ...
                    {
                        // bdtcoin: send latest proof-of-work block to allow the
    ...
    else if (strCommand == "getblocks")
    {
        ...
        for (; pindex; pindex = pindex->pnext)
        {
            if (pindex->GetBlockHash() == hashStop)
            {
                ...
                // bdtcoin: tell downloading node about the latest block if it's
